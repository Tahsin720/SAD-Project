            ...
            if password == "":
                ...
                if Profile.objects.filter(user=request.user.id).exists():
                    if User.objects.filter(id=request.user.id).exists():
                        ...
                        print("It is False: ", check)
                        if new_password != "" and re_password != "":
                            if new_password == re_password:
                                if check == True:
                                    u = Profile.objects.get(
                                ...
                                messages.error(
                                    request, 'Did not match password!')
                                return redirect('profile')
                        else:
                            if Profile.objects.filter(user=request.user.id).exists():
                                Profile.objects.filter(user=request.user.id).update(
                                    mobile_num=mobile_num, location=location)
                                User.objects.filter(id=request.user.id).update(
                                    first_name=first_name, last_name=last_name, email=email)
                                return redirect('profile')
                    else:
...
def donate_page(request):
    if request.method == 'POST':
        ...
        Book = book(isbn=isbn, name=name, author=author, user_name=user_name,
                    description=des, isdonated=isdonate, image=image, time=datetime.now())
        Book.save()
        Book_log = book_log(isbn=isbn, name=name, author=author,
                            user_name=user_name, date_time=datetime.now())
        Book_log.save()
        print('Donated!')
...
def Request_func(request):
    if request.method == 'GET':
        ...
        if not my_request.objects.filter(isbn=id, requester=name).exists():
            ...
            r.save()
            print("Req=> Save", )
            request_log = my_request_log(
                isbn=id, requester=name, name=book_name, date_time=datetime.now())
            request_log.save()
            return redirect('book')
        else:
            request_log = my_request_log(
...
def Post(request):
    if request.method == 'POST':
        ...
        Rev = Review_for_books(posted_by=uname, name=name,
                               author=author, text=text, date_time=datetime.now())
        Rev.save()
        print('Posted!')
...
def Review_page(request):
    ...
    return render(request, 'E_book_library_management_system/Review.html', {'rev_page': r_page})
...
def update_rev_func(request):
    if request.method == 'POST':
        ...
        text = request.POST['post']
        if Review_for_books.objects.filter(Id=Id).exists():
            Review_for_books.objects.filter(Id=Id).update(
                posted_by=uname, name=name, author=author, text=text)
            print('Posted!')
            return redirect('Review')
    else:
        return render(request, 'E_book_library_management_system/post.html')
    ...
    if request.method == 'POST':
        ...
        if book.objects.filter(isbn=isbn).exists():
            if len(request.FILES) != 0:
                ...
                u.image = image
                u.save()
                b = book(isbn=isbn, name=name, author=author, user_name=user_name,
                         description=des, image=image, isdonated=isdonate, time=datetime.now())
                b.save()
            else:
                book.objects.filter(isbn=isbn).update(isbn=isbn, name=name, author=author,
                                                      user_name=user_name, description=des, isdonated=isdonate, time=datetime.now())
    ...
    if request.method == 'GET':
        ...
        if not Report.objects.filter(id=Id).exists():
            if Review_for_books.objects.filter(Id=Id).exists():
                ...
                re = Report(id=rev.Id, posted_by=rev.posted_by, name=rev.name, author=rev.author,
                            text=rev.text, reporter=reporter, date_time=datetime.now())
                re.save()
                return redirect('Review')
            else:
                messages.error(request, 'No such data')
...
def payment_func(request):
    if request.method == 'POST':
        ...
        pay = payment(paid_by=paid_by, account_num=account_num, amount=amount,
                      payment_method=payment_method, date_time=datetime.now())
        pay.save()
        if amount == "100":
            print("CHECKED================>")
            p = Profile.objects.get(user=request.user.id)
            Profile.objects.filter(user=request.user.id).update(
                balance=105 + p.balance)
            return redirect('profile')
        elif amount == "1000":
            p = Profile.objects.get(user=request.user.id)
            Profile.objects.filter(user=request.user.id).update(
                balance=1200 + p.balance)
            return redirect('profile')
        elif amount == "500":
            p = Profile.objects.get(user=request.user.id)
            Profile.objects.filter(user=request.user.id).update(
                balance=650 + p.balance)
            return redirect('profile')
        elif amount == "2000":
            p = Profile.objects.get(user=request.user.id)
            Profile.objects.filter(user=request.user.id).update(
                balance=3000 + p.balance)
            return redirect('profile')
