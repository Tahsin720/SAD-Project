        ...
        if len(request.FILES) != 0:
            ...
            if password == "":
                ...
                if Profile.objects.filter(user = request.user.id).exists():
                    ...
                    # p = Profile(user = request.user.id, image = image)
                    # p.save()
...
def update_book_func(request):
    if request.method == 'POST':
        ...
        if book.objects.filter(isbn = isbn).exists():
            if len(request.FILES) != 0:
                image = request.FILES['photo']
                I = book.objects.filter(isbn = isbn)
                I.delete()
                b = book(isbn=isbn, name=name, author=author, user_name=user_name, description=des, image=image, isdonated=isdonate, time=datetime.now())
                b.save()
            else:
                book.objects.filter(isbn = isbn).update(isbn=isbn, name=name, author=author, user_name=user_name, description=des, isdonated=isdonate, time=datetime.now())    
